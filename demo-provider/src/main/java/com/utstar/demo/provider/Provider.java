...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.demo.rev150105.DataModelBuilder;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.demo.rev150105.GreetMessageBuilder;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.demo.rev150105.GreetOutput;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.demo.rev150105.GreetOutputBuilder;
...
public class Provider implements DemoService, DataTreeChangeListener<DataModel>, AutoCloseable{
    ...
    public void onDataTreeChanged(Collection<DataTreeModification<DataModel>> changes) {
        for (DataTreeModification<DataModel> change : changes) {
            ...
            GreetMessageBuilder messageBuilder = new GreetMessageBuilder()
                    .setMessage(rootNode.getDataAfter().toString());
            switch (rootNode.getModificationType()) {
                case WRITE:
                    logger.info("WRITE: onDataTreeChanged - Demo config with path {} was added or replaced: "
                            + "old Demo: {}, new Demo: {}", change.getRootPath().getRootIdentifier(),
                            rootNode.getDataBefore(), rootNode.getDataAfter());
                    publishDataChange(rootNode, messageBuilder);
                    break;
                    
                case SUBTREE_MODIFIED:
                    logger.info("SUBTREE_MODIFIED: onDataTreeChanged - Demo config with path {} was added or replaced: "
                            + "old Demo: {}, new Demo: {}", change.getRootPath().getRootIdentifier(),
                            rootNode.getDataBefore(), rootNode.getDataAfter());
                    publishDataChange(rootNode, messageBuilder);
                    break;
                ...
                case DELETE:
                    logger.info("DELETE: onDataTreeChanged - Demo config with path {} was added or replaced: "
                            + "old Demo: {}, new Demo: {}", change.getRootPath().getRootIdentifier(),
                            rootNode.getDataBefore(), rootNode.getDataAfter());
    ...
    public Future<RpcResult<GreetOutput>> greet(GreetInput input) {
        ...
        DataModelBuilder dataModelBuilder = new DataModelBuilder().setAttribute2(input.getInputdata());
